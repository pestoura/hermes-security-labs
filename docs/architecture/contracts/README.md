...
| Active authorization reference | `TB1` | Hermes control plane | Hermes → execution gateway | Typed Kali MCP and RoE epics (`EPIC-03`, `EPIC-28`) | repository-level contract and verifier implemented: signed, strict, control-plane issued authorization receipt with domain separation `hex0r.tb1.authorization.v1`, deterministic canonical reference derivation, purpose-bound public-key-only trust store and Ed25519/ECDSA-P256-SHA256 verification ([`platform/roe-contract/authorization_receipt.py`](../../../platform/roe-contract/README.md)); the execution gateway only verifies and consumes it and never issues one; Hermes issuance runtime `NOT_IMPLEMENTED`, deployed validation `NOT_RUN`; not `FINAL` | invalid, expired, not-yet-valid, wrong-purpose or scope-mismatched reference is refused before dispatch; a naked reference without a signed receipt is always refused |
