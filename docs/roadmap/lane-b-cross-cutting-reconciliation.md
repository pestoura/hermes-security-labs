...
| 4 | Connect the calibrated AI/MCP runtime to Runner Protocol and record the divergence | — | no | **Controlled composition delivered by CHG-HSL-093**: the pure projection is composed with the calibrated dispatch through an explicitly injected executor and durable idempotency, with repository tests using in-memory transport only. `PASS_CONTROLLED_IN_PROCESS` does not promote production: live network execution remains `NOT_RUN`, supervision `NOT_COMPOSED`, sandbox `NOT_IMPLEMENTED`, and promotion blocked. |
