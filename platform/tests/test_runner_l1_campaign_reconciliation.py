...
def test_policy_observation_distinguishes_historical_userns_from_current_state() -> None:
    policy = _observations()["OBS-RUNNER-POLICY-PROMOTION"]
    evidence = policy["evidence"]

    assert "userns:PASS(HISTORICAL-CHG-HSL-038;current=BLOCKED/ROOT_REQUIRED)" in evidence
    assert "trust-store:ABSENT" in evidence
    assert "signer:NOT_OBSERVED" in evidence
    assert "peer-negative:NOT_PROVEN/BLOCKED" in evidence
    ...
    assert "pre-promotion-package:NOT_RUN(HISTORICAL-CHG-HSL-038)->ASSEMBLED/HOLD/INCOMPLETE(CHG-HSL-071)" in evidence
    assert "post-effect-package:NOT_RUN" in evidence
    assert "DISABLED/NOT_RUN" in evidence

    summary = policy["summary"]
    assert "historical userns PASS" in summary
    assert "current Runner PID re-attestation is BLOCKED/ROOT_REQUIRED" in summary
    assert "LAB_L1 excludes PROD-only WORM/tenant gates" in summary


def test_chg_hsl_071_reconciles_custody_evidence_without_promotion() -> None:
    ...
    assert "hash-chain-seal:GREEN-REPO(CHG-042)" in evidence
    assert "audit-sink:GREEN-REPO(CHG-043)" in evidence
    assert "profile-aware-pre-post:GREEN-REPO(CHG-044)" in evidence
    assert "peer-identity-audit:GREEN-REPO(CHG-045)" in evidence
    assert "package-composition:GREEN-REPO(CHG-046)" in evidence
    ...
    assert "already-produced custody evidence" in summary
...
def test_status_doc_tracks_chg071_and_current_userns_boundary() -> None:
    text = STATUS_PATH.read_text(encoding="utf-8")
    assert "CHG-HSL-071" in text
    assert "HASH_CHAIN_SEAL` VERIFIED" in text
    assert "PRE_PROMOTION `ASSEMBLED / HOLD / INCOMPLETE`" in text
    assert "current Runner PID re-attestation" in text
    assert "BLOCKED / ROOT_REQUIRED" in text
    assert "Runner socket confirmed present" in text
    assert "PROD-only readiness" in text
    assert "not current LAB_L1 blockers" in text


def test_status_doc_preserves_bridge_and_reobservation_provenance() -> None:
    text = STATUS_PATH.read_text(encoding="utf-8")
    ...
    assert "NOT re-attested" in text
