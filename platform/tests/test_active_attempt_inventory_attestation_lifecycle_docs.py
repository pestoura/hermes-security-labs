...
def test_attestation_never_claims_production_dispatch_or_execution_authority() -> None:
    text = EPIC_09.read_text(encoding="utf-8")
...
def test_synthetic_runtime_evidence_is_recorded_without_final_promotion() -> None:
    text = EPIC_09.read_text(encoding="utf-8")

    assert "PR #204" in text
    assert "PR #205" in text
    assert "PR #206" in text
