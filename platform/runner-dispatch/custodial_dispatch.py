...
This layer composes the existing authenticated router with the existing Runner
outcome custody bridge. It deliberately does not move evidence persistence into
an effect adapter: adapters remain responsible for target-bound effect + durable
idempotency, while this composition layer is responsible for custodial success.

A completed effect is not returned as operational success until its terminal
Runner outcome has an immutable execution manifest and Evidence Plane projection.
If custody fails after the effect, retrying the exact logical request may call the
adapter again, but the adapter ledger must return REPLAY_SAME rather than repeat
the target effect; custody is then retried against the same deterministic
execution_id.
"""
    ...
    *,
    ...
    if evidence_custody is None or not hasattr(evidence_custody, "persist"):
        raise CustodialDispatchError(
            ...
            "Runner outcome custody component is required",
        )
