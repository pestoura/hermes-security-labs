...
def build_threat_profile(*, critical_function: str, knowledge_snapshot_id: str, actor_refs: Iterable[str], objectives: Iterable[str]) -> dict[str, Any]:
    ...
    seed = {"critical_function": critical_function, "knowledge_snapshot_id": knowledge_snapshot_id, "actor_refs": actors, "objectives": goals}
    return {"schema_version": "1.0", "profile_id": f"tp_{_digest(seed)[:32]}", **seed, "executable": False}
...
def build_emulation_plan(*, profile_id: str, critical_function: str, steps: list[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {"profile_id": profile_id, "critical_function": critical_function, "steps": normalized, "state": "PLAN_ONLY", "executable": False, "authorization_source": "CONTROL_PLANE_ONLY"}
...
def find_paths(nodes: Iterable[Mapping[str, Any]], edges: Iterable[Mapping[str, Any]], *, start: str, end: str, evidenced_only: bool = False) -> list[list[str]]:
    node_list = [dict(node) for node in nodes]
    edge_list = [dict(edge) for edge in edges]
