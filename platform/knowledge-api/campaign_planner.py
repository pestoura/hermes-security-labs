...
CONTEXT_FIELDS = {
    ...
    "asset_ids",
    "threat_technique_ids",
    "vulnerability_ids",
    "allowed_capability_ids",
    "max_intrusiveness_level",
    "minimum_confidence",
    "constraint_effect",
...
def _reject_forbidden_fields(value: Any, label: str) -> None:
    forbidden = _walk_keys(value).intersection(FORBIDDEN_FIELDS)
    if forbidden:
...
def _bounded_unique_strings(
    values: Sequence[str], *, label: str, pattern: re.Pattern[str] | None = None, allow_empty: bool = True
) -> list[str]:
    if isinstance(values, (str, bytes)) or not isinstance(values, Sequence):
        raise CampaignPlannerError(f"{label} must be a bounded list")
    items = list(values)
    if len(items) > MAX_ITEMS or (not allow_empty and not items):
        raise CampaignPlannerError(f"{label} must be a bounded list")
...
def _context_seed(
    *,
    ...
    return {
        ...
        "threat_technique_ids": sorted(threat_technique_ids),
...
def build_planning_context(
    *,
    ...
    if not SNAPSHOT_ID_RE.fullmatch(knowledge_snapshot_id):
    ...
    if not SHA256_RE.fullmatch(roe_contract_payload_sha256):
...
def _candidate_seed(
    *,
    ...
    vulnerability_ids: Sequence[str],
    knowledge_record_ids: Sequence[str],
    confidence: float,
    rationale: str,
) -> dict[str, Any]:
    return {
        ...
        "vulnerability_ids": sorted(vulnerability_ids),
        "knowledge_record_ids": sorted(knowledge_record_ids),
...
def build_candidate(
    *,
    ...
    records = _bounded_unique_strings(
        knowledge_record_ids, label="candidate knowledge record ids", pattern=RECORD_ID_RE, allow_empty=False
    )
    if len(records) > MAX_RECORD_REFS:
        raise CampaignPlannerError("candidate knowledge provenance is too large")
...
def _candidate_reasons(context: Mapping[str, Any], candidate: Mapping[str, Any]) -> list[str]:
    ...
    threat_constrained = bool(context["threat_technique_ids"] or context["vulnerability_ids"])
    ...
    if threat_constrained and not technique_match and not vulnerability_match:
...
def derive_plan(*, context: Mapping[str, Any], candidates: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for candidate in validated:
        ...
        if reasons:
            excluded.append({"candidate_id": candidate["candidate_id"], "reasons": sorted(reasons)})
            continue
    ...
    def sort_key(item: Mapping[str, Any]) -> tuple[Any, ...]:
        match_count = len(item["matched_technique_ids"]) + len(item["matched_vulnerability_ids"])
        return (
            -match_count,
...
def diff_plans(*, previous: Mapping[str, Any], current: Mapping[str, Any]) -> dict[str, Any]:
    ...
    previous_ids = [item["candidate_id"] for item in previous.get("selected_steps", [])]
    current_ids = [item["candidate_id"] for item in current.get("selected_steps", [])]
    ...
    common_previous = [item for item in previous_ids if item in set(current_ids)]
    common_current = [item for item in current_ids if item in set(previous_ids)]
    ...
    body = {
        ...
        "common_order_changed": order_changed,
