...
## Isolation
- Runs in a dedicated Docker bridge network: `juice-shop-lab`
...
- Node.js available on host (for smoke test)
...
```bash
docker network connect juice-shop-lab hermes-kali-mcp
```
...
```bash
docker network disconnect juice-shop-lab hermes-kali-mcp
```
...
- Free GitHub Actions quota may be exhausted for private repo
