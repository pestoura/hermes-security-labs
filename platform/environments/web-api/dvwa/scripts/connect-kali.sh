...
[[ "${app_state}" == running ]] || fail "DVWA container is not running"
...
[[ "${app_present}" == true ]] || fail "expected DVWA endpoint ${app_name} is missing"

if docker network inspect "${DB_NETWORK}" >/dev/null 2>&1; then
  db_endpoints="$(docker network inspect "${DB_NETWORK}" --format '{{range .Containers}}{{.Name}}{{"\n"}}{{end}}')"
  if grep -Fxq "${KALI_CONTAINER}" <<<"${db_endpoints}"; then
    fail "Kali is unexpectedly attached to internal database network ${DB_NETWORK}"
  fi
fi

if [[ "${kali_present}" == true ]]; then
  echo "[connect-kali] ALREADY CONNECTED"
  echo "[connect-kali] Authorised target: http://dvwa:80/login.php"
  exit 0
fi
