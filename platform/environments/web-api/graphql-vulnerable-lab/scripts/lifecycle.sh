...
start(){ "${COMPOSE[@]}" config --quiet; if [ -z "$(container_id)" ] && ss -ltn "sport = :${HOST_PORT}" | grep -q LISTEN; then echo "[start] Port ${HOST_PORT} already in use"; return 1; fi; "${COMPOSE[@]}" up -d --build; local id health; for _ in $(seq 1 48); do id="$(container_id)"; if [ -n "${id}" ]; then health="$(docker inspect "${id}" --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}}')"; [ "${health}" = healthy ] && { echo '[start] Healthy'; "${COMPOSE[@]}" ps; return 0; }; [ "$(docker inspect "${id}" --format '{{.State.Status}}')" = exited ] && break; fi; sleep 5; done; "${COMPOSE[@]}" logs --tail 100; return 1; }
