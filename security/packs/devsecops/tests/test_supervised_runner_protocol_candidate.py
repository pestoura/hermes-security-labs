...
from devsecops_runbooks.supervised_runner_protocol_adapter import (  # noqa: E402
    WORKER,
...
ADAPTER = PACK_SRC / "devsecops_runbooks" / "supervised_runner_protocol_adapter.py"
...
def _candidate(database: Path) -> SupervisedSyntheticDevSecOpsRunnerCandidate:
    return SupervisedSyntheticDevSecOpsRunnerCandidate(
...
def test_success_uses_devsecops_worker_and_replays_without_second_process(
    tmp_path: Path,
) -> None:
    database = tmp_path / "success.sqlite3"
    request = _request(
        "conformance.process.success",
        key="devsecops-supervised:success:0001",
        input_data={
    ...
    assert supervision["stdout_sha256"] == hashlib.sha256(
        b"synthetic-devsecops-supervised-success\n"
    ).hexdigest()
    assert first.effect_count == 1
    serialized = json.dumps(first_outcome, sort_keys=True)
    assert "synthetic-devsecops-supervised-success" not in serialized
    ...
    assert first_outcome["evidence_refs"][0]["uri"].startswith(
        "evidence://devsecops-supervised-runner-candidate/"
    )
...
def test_nonzero_worker_exit_is_normalized_without_raw_stderr(tmp_path: Path) -> None:
    ...
    outcome = _outcome(
        candidate.dispatch(
            _request(
                ...
                key="devsecops-supervised:failure:0001",
            )
        )
    )
    ...
    assert "synthetic-devsecops-supervised-failure" not in json.dumps(
...
def test_timeout_and_cancellation_are_bounded_and_cleanup_internal_state(
    ...
    timeout_outcome = _outcome(
        timeout_candidate.dispatch(
            _request(
                ...
                key="devsecops-supervised:timeout:0001",
                hard_timeout_ms=300,
                grace_period_ms=50,
            )
        )
    ...
    request = _request(
        ...
        key="devsecops-supervised:cancel:0001",
    ...
    assert not list(tmp_path.glob(".devsecops-ready-*.pid"))
...
def test_descendant_residue_is_inconclusive_and_removed(tmp_path: Path) -> None:
    ...
    outcome = _outcome(
        candidate.dispatch(
            _request(
                ...
                key="devsecops-supervised:residue:0001",
            )
        )
    )
    ...
    assert not list(tmp_path.glob(".devsecops-residue-*.pid"))
...
def test_real_capability_and_authorization_are_refused_before_claim(
    ...
    real_capability = _request(
        "devsecops.sca.scan",
        key="devsecops-supervised:real-capability:0001",
    )
    ...
    real_authorization = _request(
        "conformance.process.success",
        key="devsecops-supervised:real-authorization:0001",
...
def test_wrapper_is_pack_isolated_and_shared_engine_owns_lifecycle() -> None:
    ...
    assert WORKER == (
        PACK_SRC / "devsecops_runbooks" / "synthetic_supervised_worker.py"
    ).resolve()
