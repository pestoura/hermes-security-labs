"""Typed request/result/evidence contracts for AI/MCP handler execution.

Every adapter must return an :class:`ExecutionResult`; the runner serialises
it into a stable, sanitised JSON document. Untrusted payload material is
validated in exactly one place: :meth:`ExecutionRequest.from_payload`.
"""
...
class Evidence:
    ...
    ``value`` never carries prompt text, model output or synthetic markers:
    adapters report rule identifiers, counts, hashes, status codes and flags.
    """
...
class ExecutionRequest:
    ...
    def from_payload(cls, payload: Any) -> ExecutionRequest:
        ...
        This is the single validation point for untrusted input. It never
        widens an internal allowlist: it only produces a typed request that
        the policy layer subsequently authorises.
        """
